...
export class PrivecyPolicyController {
  ...
  @Post()
  async create(
    @Body() createPrivecyPolicyDto: CreatePrivecyPolicyDto,
    @CompanyId() companyId: string,
  ) {
    const data = await this.privecyPolicyService.create(
  ...
  async update(
    @Param('id', ParseIntPipe) id: number,
    @Body() updatePrivecyPolicyDto: UpdatePrivecyPolicyDto,
    @CompanyId() companyId: string,
  ) {
    const data = await this.privecyPolicyService.update(
      id,
      updatePrivecyPolicyDto,
      companyId,
    );
    return {
      status: 'success',
