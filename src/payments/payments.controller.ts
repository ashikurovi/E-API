...
export class PaymentsController {
  ...
  async sslCallback(@Query() query: any) {
    if (query?.val_id) {
      const validation = await this.paymentsService.validateSslPayment(
        query.val_id,
      );
  ...
  async cashOnDelivery(
    @Body() body: { orderId: string; amount: number; address: string },
  ) {
    const data = await this.paymentsService.createCashOnDelivery(
      body.orderId,
      body.amount,
      body.address,
    );
