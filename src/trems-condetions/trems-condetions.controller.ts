...
export class TremsCondetionsController {
  constructor(
    private readonly tremsCondetionsService: TremsCondetionsService,
  ) {}

  @Post()
  async create(
    @Body() createTremsCondetionDto: CreateTremsCondetionDto,
    @CompanyId() companyId: string,
  ) {
    const data = await this.tremsCondetionsService.create(
      createTremsCondetionDto,
      companyId,
    );
    return {
      status: 'success',
  ...
  async update(
    @Param('id', ParseIntPipe) id: number,
    @Body() updateTremsCondetionDto: UpdateTremsCondetionDto,
    @CompanyId() companyId: string,
  ) {
    const data = await this.tremsCondetionsService.update(
      id,
