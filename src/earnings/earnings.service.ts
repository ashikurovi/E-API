...
export class EarningsService {
  ...
  async getEarningsOverview() {
    ...
    const yearInvoices = allInvoices.filter(
      (invoice) => invoice.createdAt >= yearStart,
    );
    ...
    const lastMonthTotal =
      monthlyEarnings[monthlyEarnings.length - 1]?.totalPNL || 0;
    const previousMonthTotal =
      monthlyEarnings[monthlyEarnings.length - 2]?.totalPNL || 0;
    const earningsDelta =
    ...
    return {
      ...
      channelBreakdown: Object.entries(channelBreakdown).map(
        ([name, amount]) => ({
          name,
          amount,
        }),
