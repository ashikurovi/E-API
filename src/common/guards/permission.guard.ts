...
export class PermissionGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    const requiredPermission =
      this.reflector.getAllAndOverride<FeaturePermission>(PERMISSION_KEY, [
        context.getHandler(),
        context.getClass(),
      ]);
    ...
    if (!user?.permissions || !Array.isArray(user.permissions)) {
      throw new ForbiddenException(
        'Insufficient permissions: User does not have required permissions',
      );
    }

    if (!user.permissions.includes(requiredPermission)) {
      throw new ForbiddenException(
        `Insufficient permissions: Required permission '${requiredPermission}' not found`,
      );
    }
