...
export class DashboardService {
  ...
  private async getSalesOverviewByPeriod(
    ...
    paidOrders.forEach((order) => {
      ...
      bucketMap.set(
        key,
        (bucketMap.get(key) || 0) + Number(order.totalAmount || 0),
      );
    });
  ...
  async getStatistics(companyId: string) {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const earning = dayOrders.reduce(
        (sum, o) => sum + Number(o.totalAmount || 0),
        0,
      );
    ...
    const paymentData = topCustomers.map((customer, index) => {
      ...
      const productName =
        latestOrder?.items?.[0]?.product?.name || 'Payment Page';
   ...
   */
  async getAiDailyReport(
    companyId: string,
  ): Promise<{ report: string; generatedAt: string }> {
    const groqApiKey = process.env.AIDAILYREPORT ?? '';
    if (!groqApiKey) {
      return {
        report:
          'AI report is not configured. Please add AIDAILYREPORT to your environment variables. Get a free API key at https://console.groq.com',
...
Generate a brief daily report with: 1) Executive summary, 2) Key highlights, 3) One actionable recommendation.`;
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are a helpful business analyst. Generate concise, professional daily reports for e-commerce store owners.',
            },
   ...
   */
  async getAiLiveMessages(companyId: string): Promise<{
    messages: Array<{ text: string; type: string; timestamp: string }>;
    generatedAt: string;
  }> {
        ...
        directions: [
          {
            title: 'Configure AI',
            action:
              'Add AISALE to your environment to enable sales direction. Get free key at console.groq.com',
            priority: 'info',
          },
        ],
        generatedAt: new Date().toISOString(),
...
Recent orders: ${JSON.stringify(recentOrders.slice(0, 3))}`;
          ...
          messages: [
            {
              role: 'system',
              content:
                'You output only valid JSON arrays. Be practical and actionable for store owners.',
            },
            { role: 'user', content: prompt },
          ],
          max_tokens: 500,
          ...
          : [
              {
                title: 'Stay active',
                action:
                  'Monitor your dashboard and respond to new orders promptly',
                priority: 'medium',
              },
            ],
        generatedAt: new Date().toISOString(),
  ...
  async suggestAiDescription(
    ...
    const typePrompts: Record<string, string> = {
      product:
        'Generate a concise, compelling product description for an e-commerce listing. Include key features, benefits, and call-to-action. Keep it under 200 words.',
      promocode:
        'Generate a short, clear description for a promo/discount code. Explain what the offer is and who it applies to. Keep it under 100 words.',
      package:
        'Generate a professional package/plan description for a subscription or service tier. Highlight features and value. Keep it under 150 words.',
      general:
        'Generate a clear, professional description based on the context. Keep it concise and under 150 words.',
    };
    ...
    const langInstructions: Record<string, string> = {
      ...
      'bn-Latn':
        'Write the description in Minglish (Bengali/Bangla written in English/Latin letters, e.g. "ami bhat khabo" instead of "আমি ভাত খাবো").',
    };
...
Return ONLY the description text, no quotes or extra formatting.`;
          ...
          messages: [
            {
              role: 'system',
              content:
                'You generate concise, professional descriptions for e-commerce. Return only the description text.',
            },
            { role: 'user', content: prompt },
          ],
          max_tokens: 400,
  ...
  async translateReport(
    ...
    const langInstructions: Record<string, string> = {
      ...
      'bn-Latn':
        'Translate the following text to Minglish (Bengali written in Latin/English letters). Return ONLY the translated text, no explanation.',
      minglish:
        'Translate the following text to Minglish (Bengali written in Latin/English letters). Return ONLY the translated text, no explanation.',
