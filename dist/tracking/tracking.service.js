...
let TrackingService = class TrackingService {
    ...
    async trackRedX(trackingId) {
        try {
            const res = await fetch(`https://api.redx.com.bd/v1/logistics/global-tracking/${encodeURIComponent(trackingId)}`);
            if (!res.ok)
                return null;
    ...
    async trackSteadfast(trackingId) {
        try {
            const res = await fetch(`https://www.steadfast.com.bd/track/consignment/${encodeURIComponent(trackingId)}`);
    ...
    async trackAnywhere(trackingId) {
        ...
        const result = (await this.trackRedX(trimmed)) ||
