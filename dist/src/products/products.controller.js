...
let ProductController = class ProductController {
    ...
    async create(createDto, companyIdFromQuery, companyIdFromToken, req) {
        try {
            ...
            const performedByUserId = role &&
                [
                    system_user_role_enum_1.SystemUserRole.SUPER_ADMIN,
                    system_user_role_enum_1.SystemUserRole.SYSTEM_OWNER,
                    system_user_role_enum_1.SystemUserRole.EMPLOYEE,
                ].includes(role)
                ? numericUserId
                : undefined;
        ...
        try {
            ...
            if (fileExtension === '.csv') {
                ...
                products = data.map((row) => {
                    ...
                    const stockVal = normalizedRow.stock ??
                    ...
                    return {
                        ...
                        stock: stockVal !== undefined && stockVal !== ''
                            ? parseInt(String(stockVal), 10) || 0
                            : undefined,
                    };
                });
    ...
    async update(id, updateDto, companyIdFromQuery, companyIdFromToken, req) {
        ...
        if (!companyId)
            throw new common_1.BadRequestException('companyId is required');
        const performedByUserId = req?.user?.role &&
